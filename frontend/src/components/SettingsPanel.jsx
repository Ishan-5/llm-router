...
export default function SettingsPanel({ onClose, onSaved }) {
  ...
  const [tiers, setTiers] = useState(() => {
    // initialise from localStorage immediately so dropdowns are correct on open
    const saved = loadFromLocalStorage()
    const init = { cheap: { ...EMPTY_TIER }, mid: { ...EMPTY_TIER }, frontier: { ...EMPTY_TIER } }
    TIERS.forEach((t) => {
      if (saved[t]) {
        const isCustom = saved[t].model_id && ![''].includes(saved[t].model_id)
        init[t] = {
          ...EMPTY_TIER,
  ...
  async function handleSave() {
    ...
    try {
      ...
      // persist to localStorage -- store is_custom flag so we can restore dropdown state
      const localStore = {}
      for (const tier of TIERS) {
        const t = tiers[tier]
        if (!t.enabled) continue
  ...
  async function handleSave() {
    ...
    try {
      ...
      // store non-sensitive parts in localStorage (no api_key)
      const localStore = {}
      for (const tier of TIERS) {
        const t = tiers[tier]
        if (!t.enabled) continue
        localStore[tier] = {
          provider: t.provider,
          model_id: getModelId(tier),
          api_key: t.api_key,   // key stays in localStorage only, never in DB
          enabled: true,
        }
      }
      localStorage.setItem('byom_config', JSON.stringify(localStore))
